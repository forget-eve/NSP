...
- [x] 什么是随机数？基于已学课程大致描述随机数的作用？
	> - 随机数是一种统计上独立且无偏的二进制数字序列
	> - 随机数作用:
	> > - 作为分组密码技术中的密钥
	> > - 作为流密码技术中的密钥或密钥流
	> > - 用于产生公钥密钥算法中的⼤素数、私钥等，如RSA中p和q的选取
	> > - 用于认证协议中防止重放攻击
...
- [x] 基本的安全模型、安全体系结构
	...
	- 网络安全模型(安全通道建立，数据传输)、网络访问安全模型(访问控制、权限管理、安全通道建立、数据传输)

- [x] 基本的加密、认证、密钥交换算法
	- 用户认证(基于挑战应答：对称加密、签名、非对称加密、HMAC)
	- 数据源认证(签名、HMAC)
	- 数据机密性、完整性保护等的基本方法
...
- [x] 分组密码算法的工作模式
	- IV值的使用和作用(即使相同明文亦可得到不同密文)
	- 典型加密模式的操作和差错的影响

## 公钥基础设施PKI

- [x] PKI基本概念：什么是？为什么要有？基本功能和其他功能

- [x] PKI体系基本组成
	- CA、RA、CRL

- [x] 证书的基本结构和基本内容、其他主要内容
