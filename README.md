...
- [x] 密码学课程中涉及的常见加密算法
	- [对称](#kaodian11)、[非对称](#kaodian10)(包括RSA，椭圆曲线密码，ELGamal密码，[DH密钥交换(重点)](#DH))、哈希函数(主要记忆 [长度大小](#hash) 和 [强弱碰撞性](#crush) )、HMAC等等
		> - 开销一般为：非对称>>对称>>哈希>>异或
		> - 非对称协议一般情况： 协商参数→非对称加密→DH交换密钥→建立安全通道

- [x] 分组密码算法的工作模式
	- IV值的使用(收发两方透过事前约定的机制自行计算出对应的初始向量(不过，实现上还是经常会把nonce送过去以便检查消息的遗漏))和作用(即使相同明文亦可得到不同密文)
	- 典型加密模式的操作和差错的影响([ECB](#ECB)，[CBC](#CBC)，[CFB](#CFB)，[OFB](#OFB)，[CRT](#CRT))
...
- [x] 证书生命周期(初始化(实体注册，密钥对产生，[证书创建](#kaodian19)，证书分发，密钥备份) -> 使用 -> 撤销)、 [证书链](#kaodian17)(由不同CA创建的证书序列构成，每个连续的证书都是由CA颁发的证明下一个CA公钥可信任的证书) 、交叉认证(多个PKI独立地运行, 相互之间应建立信任关系：多个PKI独立运行相互建立信任关系，两个CA间互相认证颁发证书)
	- 如何产生一个证书，获得证书的基本流程
		...
		- 证书获取的方式：
			...
			> - 从访问发布证书的目录服务器获得
			> - 从其他公共站点的共享位置获得
	- [如何验证证书](#kaodian18)(拆封证书(用于获得签名并验证)->证书链认证(直到信任锚点)->CRL查询(查看证书是否被撤销)->字段验证(包括序列号验证、有效期认证、证书使用策略认证等))
...
- [x] 安全关联SA(包含的基本内容)、SAD、SPD

- [x] AH/ESP在源端和目的端的处理流程
...
- [x] IKE(不拘泥于协议细节的记忆)
	- 目的(基于长期密钥协商得到短期密钥)
	- 不同模式存在的原因、大致流程(主模式和野蛮模式)
		- 参数协商、相互认证、密钥协商
	- 两个阶段(V1：ISAKMP SA，IPSec SA；V2：IKE SA，Child SA)
...
- [x] NAT基本原理，作用
	- SNAT、DNAT的基本作用、内核中处理位置
	- 典型应用数据访问过程中的数据包地址和端口信息变化
