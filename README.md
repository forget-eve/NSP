...
##### 密码分组链接(<span id="CBC">CBC</span>, Cipher-block chaining)模式
...
- [x] <span id="kaodian12">什么是PKI呢？</span>
...
- [x] IPSec的体系结构文档规定：当两个传输模式的SA被捆绑在⼀起以允许AH协议和ESP协议可以在同⼀个端对端流中实现时，仅有⼀个顺序看起来是较为合理的————先执行ESP协议再执行AH协议。请问：为什么推荐这种顺序而不是先认证后加密？
	> - **在解密前对包进行筛选，减少计算开销，类似先加密后签名**
...
- 1. 分段(将上层消息分割为不大于 $2^{14}bytes$ 的块)
- 2. 压缩(默认为空算法)
- 3. 添加MAC(完整性保护)
- 4. 加密(机密性保护)
- 5. 添加SSL记录协议头
...
$$NAT-PT:过载(Overloading) \leftarrow 一对多$$
...
- **服务质量(QoS)保证**
	- [x] 对不同的用户提供不同的服务质量，如带宽、时延等保证，这取决于广域网上是否提供QoS保证
...
- `电子邮件安全协议`
	...
	- POP3和IMAP是什么？
		- [x] Post Office Protocol 3 和 Internet Mail Access Protocol 都能够让客户端下载服务器上的邮件。
...
<p align="center">
  <img src="./img/Base64.png" alt="Base64">
	<br>
	<br>
	<img src="./img/Base641.png" alt="Base64">
</p>

- [x] 什么是 Base64 变换？
	- 一种将二进制数据转化为 ASCII 字符的变换

- [x] 为什么邮件要使用 Base64 变换？
...
- [x] PGP体制中，前一次会话密钥生成后，希望生成多少会话密钥？
	- 在PGP（Pretty Good Privacy）体制中，会话密钥的生成是与每个会话（每次加密操作）相关的，因此每次需要加密新的数据时，都会生成一个新的会话密钥。PGP体制使用混合加密的方式，其中会话密钥用于对称加密（例如AES），而会话密钥本身则使用接收者的公钥进行非对称加密（例如RSA）。
	- 具体来说：
	- 每次加密操作生成一个新的会话密钥：
		- 当你需要加密一段新的数据时，PGP会生成一个新的随机会话密钥。
		- 这个会话密钥用于对称加密你的数据。
		- 会话密钥然后使用接收者的公钥进行加密，并附加在加密后的消息中。
	- 独立于之前的会话密钥：
		- 新生成的会话密钥是独立于之前任何会话的密钥的。每次会话密钥都是随机生成的，并且不同会话之间没有关联。
		- 这意味着即使之前的会话密钥被泄露或破解，新生成的会话密钥和会话数据仍然是安全的。
	- 实际应用
		- 在实际应用中，前一次会话密钥生成后，每次需要加密新的数据时都会生成新的会话密钥。例如：
			> - 邮件加密：每次发送加密邮件时，PGP生成一个新的会话密钥来加密该邮件内容。
			> - 文件加密：每次加密一个新的文件时，PGP生成一个新的会话密钥来加密该文件内容。
	- 生成多少会话密钥
		- PGP并不预先确定生成多少会话密钥，而是根据加密操作的需要动态生成。因此：
			> - 如果你进行一次加密操作，就会生成一个会话密钥。
			> - 如果你进行多次加密操作，就会生成相应数量的会话密钥，每次一个。
	- 总结来说，前一次会话密钥生成后，新的会话密钥会在每次需要加密新的数据时生成，没有固定的数量限制，完全取决于加密操作的频率和需求。

- [x] 具有N个公钥的用户至少有一个重复密钥ID的几率是多少？
	- 密钥不重复的概率 $P(N)=\frac{2^{64}}{2^{64}}·\frac{2^{64}-1}{2^{64}}·\dots ·\frac{2^{64}-(N-1)}{2^{64}}=\frac{2^{64}!}{(2^{64})^N(2^{64}-N)!}$
	- 至少一个重复的概率 $1-P(N)$
...
- MIME的内容类型和其传输编码之间的区别？
	- [x] 内容类型用于描述信息的表述方法，传输编码是内容使用的编码方案

- 无损压缩算法（zip）用于S/MIME，为什么在压缩前签名会更好？
	- [x] 签名后再压缩是为了方便保存未压缩的报文和签名，为了以后存储方便
...
<p align="center">
  <img src="./img/DNSSEC引入新的资源记录.png" alt="DNSSEC引入新的资源记录">
</p>

### DNSSEC实施后的完整域名解析过程

<p align="center">
  <img src="./img/DNSSEC实施后的完整域名解析过程.png" alt="DNSSEC实施后的完整域名解析过程">
</p>
...
- [x] 安全电子交易协议SET(Secure Electronic Transaction)是由Visa和Master Card所开发的，为了在Internet上进行在线交易时，保证信用卡支付的安全性而设计的开放规范，已得到IBM、HP、Microsoft等大公司的支持，已成为事实标准，并获得IETF标准认可(RFC3538)

- [x] SET提供了 `消费者、商家和银行` 之间的认证，确保了网上交易数据的 **保密性** ，数据的 **完整性** 以及交易的 **不可抵赖性** 。特别是能 **保证不将消费者银行卡号暴露给商家，不将消费者的购物信息暴露给银行** 等优点，因此它成为目前公认的信用卡/借记卡网上交易安全标准

- [x] SET采用公钥密码体制，遵循X.509数字证书标准
...
- `收单行` (Aquirer):为商家建立帐户并处理支付卡授权和支付的金融机构
...
- **确保采用最好的安全系统和技术来保护电子商务交易中的所有合法参与者的利益**

- **所产生的协议要既不依赖于传输安全机制，又不妨碍它们的使用**

- **促进并鼓励软件与网络提供商之间的互操作**
	- SET协议和格式是独立于硬件平台、操作系统和WEB软件
...
<p align="center">
  <img src="./img/SET处理流程.png" alt="SET处理流程">
</p>

### 双重数字签名

- [x] 发送者寄出 **两个相关信息** 给接收者，对这两组相关信息，接收者 **只能解读其中一组** ，另一组只能转送给第三方接收者，不能打开看其内容。这时发送者就需分别加密两组密文，并针对两组明文信息联系起来并进行签名，称其 `双重数字签名` 。

- [x] 应用场合 ：电子商务购物、付款。是SET和non-SET中常用

#### Dual Signature

<p align="center">
  <img src="./img/Dual-Signature.png" alt="Dual Signature">
</p>

#### 持卡人发送购买请求(Purchase request)

<p align="center">
  <img src="./img/持卡人发送购买请求.png" alt="持卡人发送购买请求">
</p>

#### 商家验证过程

<p align="center">
  <img src="./img/商家验证过程.png" alt="商家验证过程">
</p>

#### 电子交易有两大体系

- $\color{blue}{SET}$
	- SET Secure Electronic Transaction 安全电子交易协议，为了在网上进行交易时，保证信用卡支付的安全而设计的一个开放的规范

- $\color{blue}{Non \ SET}$
	- 相对于SET系统而言的如SSL等协议基础上的交易系统

#### SET & SSL

- [x] 事实上，SET和SSL不是同一类型的协议，但由于两者都被用来提供电子商务中的安全保障，因此，要对这两类系统作些比较。

- [x] SET是一种专门用于电子交易的安全协议，它详细定义了加密信息的格式和完成信用卡电子交易过程中各方传输信息的规则，定义了银行、商家、持卡人之间必须符合的协议规范。

- [x] SSL协议是传输层之上的安全协议，为通信双方建立一条安全连接，它可以支持各种不同的应用层协议。

- [x] 认证机制方面：SET的安全需求较高，参与交易的成员必须先申请数字证书来识别身份；而在SSL中只有服务器端需要认证，客户端是可选的。

- [x] 对消费者：SET保证了商家的合法性，并且用户的信用卡号不会被窃取。

- [x] 安全性：SET的安全性比SSL高。因为SET严格规定了持卡人、商家与银行间的信息规范。

- [x] SET对参与交易的各方定义了互操作接口，一个系统可以用不同厂商的产品组成。
...
- [x] 填空(18-20空)20%、选择(5单选+5不定项)20%、简答和计算(包含画图)60%
	- 其中不定项选择题一个or少选1个得1分

## 基本概念

- [x] 概念、定义、名词
	- [网络安全特征( `机密性` 、 `完整性` 、 `可用性` 、可认证、不可否认、可控性)](#kaodian1) ； [常见的不安全因素](#kaodian2) 、 [导致不安全的原因](#kaodian3) ； [常见攻击手段](#kaodian4)
	- 主要概念和术语的规范中英文

- [x] 基本的安全模型、安全体系结构
	- [安全攻击(主动攻击和被动攻击)](#kaodian5) 、 [安全机制](#kaodian6) 、 [安全服务及其关系](#kaodian7)
	- [网络安全模型(安全通道建立，数据传输)](#kaodian8)、 [网络访问安全模型(访问控制、权限管理、安全通道建立、数据传输)](#kaodian9)

- [x] 基本的加密、认证、密钥交换算法
	- 用户认证(也叫对等实体认证，基于 [挑战应答](#challenge) ：对称加密、签名、非对称加密、HMAC)
		> - 签名和加密的区别： `数据加密` 保证了 `数据接受方` 的数据安全性。 `数据签名` 保证了 `数据发送方` 的数据安全性。 [详见此处](https://blog.csdn.net/m0_57126939/article/details/121230222)，非对称加密使用公钥和私钥进行加密和解密操作，保护数据的机密性和完整性。 数字签名使用私钥对数据进行签名，验证数据的来源和完整性。
	- 数据源认证(签名、HMAC，对消息来源进行单方确认，不需要对消息发送方实体身份进行认证)
	  > - 消息认证(包括HMAC和加密，但是一般不使用加密，一般有三类四种方法，也就是对称加密，私钥加密，MAC和hash加签名)，不提供对数据的不可否认性，但是提供保护性，其能提供数据源认证能力
		> - 数字签名，可以提供消息的不可否认性，因为其是用发送方私钥获得的
	- 数据机密性、完整性保护等的基本方法
		> - 数据机密性的保护方法一般有：加密
		> - 数据完整性的保护方法一般有：HMAC(也就是消息认证码MAC+散列函数H)，签名，一般都是通过提取并对比 [`消息摘要`](#secret) 的方式来实现。

- [x] 密码学课程中涉及的常见加密算法
	- [对称](#kaodian11)、[非对称](#kaodian10)(包括RSA，椭圆曲线密码，ELGamal密码，[DH密钥交换(重点)](#DHse))、哈希函数(主要记忆 [长度大小](#hash) 和 [强弱碰撞性](#crush) )、HMAC等等
		> - 开销一般为：非对称>>对称>>哈希>>异或
		> - 非对称协议一般情况： 协商参数→非对称加密→DH交换密钥→建立安全通道

- [x] 分组密码算法的工作模式
	- IV值的使用(收发两方透过事前约定的机制自行计算出对应的初始向量(不过，实现上还是经常会把nonce送过去以便检查消息的遗漏))和作用(即使相同明文亦可得到不同密文)
	- 典型加密模式的操作和差错的影响([ECB](#电码本ecb-electronic-codebook模式)，[CBC](#密码分组链接cbc-cipher-block-chaining模式)，[CFB](#密码反馈cfb-cipher-feedback模式)，[OFB](#输出反馈ofb-output-feedback模式)，[CRT](#计数器crt-counter模式))

## 公钥基础设施PKI

- [x] PKI基本概念： [什么是？](#kaodian12) [为什么要有？](#kaodian13) [基本功能和其他功能](#kaodian14)

- [x] [PKI体系基本组成](#kaodian16)
	- CA(颁发证书)、RA(注册)、CRL(由CA进行签发，用来实现撤销或者作废证书)

- [x] [证书的基本结构和基本内容](#kaodian15)、[其他主要内容](#kaodian20)

- [x] 证书生命周期(初始化(实体注册，密钥对产生，[证书创建](#kaodian19)，证书分发，密钥备份) -> 使用 -> 撤销)、 [证书链](#kaodian17)(由不同CA创建的证书序列构成，每个连续的证书都是由CA颁发的证明下一个CA公钥可信任的证书) 、交叉认证(多个PKI独立地运行, 相互之间应建立信任关系：多个PKI独立运行相互建立信任关系，两个CA间互相认证颁发证书)
	- 如何产生一个证书，获得证书的基本流程
		- 产生证书(一般也就是指输出化过程)
		- 证书获取的方式：
			> - 发送者发送签名时，附加发送自己的证书
			...
			> - 从访问发布证书的目录服务器获得
			> - 从其他公共站点的共享位置获得
	- [如何验证证书](#证书验证)(拆封证书(用于获得签名并验证)->证书链认证(直到信任锚点)->CRL查询(查看证书是否被撤销)->字段验证(包括序列号验证、有效期认证、证书使用策略认证等))
	- 什么情况下需要撤销证书(证书过期或者在过期前撤销，比如私钥泄露、关系终止、CA签名私钥泄露或变更(以上两个阶段不需要归档))，如何撤销一个证书(证书过期->证书撤销->存档->审计信息(包括产生密钥对，证书请求，密钥泄露报告，证书中包括的某种关系的终止，证书的使用过程))
	- 公钥加密、数字签名的密钥对在证书维护上的区别
		- 公钥加密：采用 `收方的公钥` 对随机产生的会话密钥进行加密(称为 `数字信封` )，提供 `机密性`
		- 数字签名：对待发的数据首先基于 `Hash` 生成一段 `数据摘要` ，再采用 `己方私钥` 基于非对称加密算法进行加密，结果 `附在原文上` 一起发送，接收方对其进行验证，判断原文真伪。从计算开销的角度，这种数字签名方法适用于对大文件的处理。在PKI中提供 `数据完整性保护` 和提供 `不可否认性服务` (和身份认证)
	- 交叉认证:层次模型、分布式、桥式、混合
 		- 层次模型：易于控制，但是根一旦被破坏整个体系都会受影响。
		- 分布式：鲁棒性好，可拓展性好，CA间通过交叉认证在不同的CA之间建立信任。但是证书链不确定，可能会很长。更有可能出现环路。
		- 桥式：首先要建立一个桥CA，桥CA在不同的PKI体系之间起信任桥梁的作用，它不直接向用户签发证书，也不作为体系中用户的一个信任点，只用来建立一个端到端的信任关系。

- [x] 注：用户的公私密钥对的产生有三种方式，视策略而定：
	- 用户自己产生密钥对
	- CA为用户产生密钥对
	- CA之外的其他可信第三方产生密钥对

## IPSec(网络层协议)：AH、ESP与IKE

- [x] IPv4地址32bits，IPv6为128bits，并提供安全功能，实时应用支持，自动配置，移动性，安全性

- [x] [安全关联SA](#_33-ipsec中的安全组合sa)
	- 三元组(SPI(唯一标识)，用于输出处理的目的IP地址和协议如AH或ESP)、[SPI](#安全参数索引spi)、SA的单向性(由目的端确定)

- [x] [安全关联SA(包含的基本内容)、SAD(存储SA参数)](#安全关联数据库)、[SPD](#安全策略数据库)

- [x] AH(无连接的数据完整性、数据源认证、抗重放保护服务)/ESP(数据保密(对称加密)、无连接完整性(可选，不覆盖IP头标)、数据源认证、抗重放保护服务)在源端和目的端的处理流程
	- [AH处理](#ah处理)
	- [SP处理](#esp处理)

- [x] AH/ESP头标、保护范围
	- [AH认证头标、保护范围(为整个IP分组范围(除了头标中的可变域)，也就是IP头标，AH头标，TCP/UDP头标，净荷)](#认证头标)
	- [ESP认证头标、保护范围(认证范围不包括头标，加密范围包括除了IP头标和ESP头标的部分，也不包括ESP的认证数据，但是包括ESP尾标，ESP净荷为在头标和尾标之间的部分也就是传输协议和数据部分)](#esp加密和认证的范围)

- [x] [AH、ESP同时使用的顺序问题](#关于加密与认证顺序)

- [x] [工作模式：传输模式和隧道模式](#_36-ipsec的传输模式与隧道模式)
	- 传输模式主要为上层协议提供保护；隧道模式对整个IP包提供保护，隧道模式中ESP的几个范围除ESP净荷外都增加一个原IP头标
	- 建议画图说明

- [x] [IPSec与NAT产生冲突的原因](#_37-ipsec与nat)
	- 具有AH头标或ESP头标的的IP分组不能穿越NAT和NAT-PT

- [x] [IPSec VPN](#_38-ipsec隧道模式的应用vpn)

- [x] IKE(不拘泥于协议细节的记忆)(工作于应用层)
	- 目的(基于长期密钥进行双向认证并协商得到短期密钥)
	- 不同模式存在的原因、大致流程([主模式(在IKE中必须配置主模式)](#主模式main-mode)和[野蛮模式(用来简化规程和提高处理效率)](#野蛮模式aggressive-mode))
		- 参数协商、相互认证、密钥协商(密钥交换使用DH密钥交换，一共3/6个报文)
	- 两个阶段(V1：ISAKMP SA(双向)，IPSec SA(单向)；V2：IKE SA，Child SA)
	- 以IPSec IKE为例，了解安全通道的建立原理和过程
	- 从原理上理解头标的组成(可组合)和大致的功能
	- 相关密钥的推导原理和作用(随机数/派生方法)
		- [x] IKEv1密钥推导：
...
- [x] SSL的基本层次结构、安全服务
	- 底层：记录协议(封装以下三种协议或应用层数据(记录类型：20=改变密码规格，21=告警，22=握手，23=应用层数据)，建立在TCP上，使用对称加密提供保密性和使用HMAC提供完整形)
	- 上层：握手协议(立客户与服务器之间的安全通道，该协议包括双方的相互认证，交换密钥参数)、密码变化协议(告知改变密码参数)、警告协议(向对端指示其安全错误)、用户数据

- [x] SSL协议的基本描述(是一种在TCP协议之上为两个端实体(End Entity)之间提供安全通道的协议)

- [x] 安全操作流程：握手协议、会话重用(返回相同SessionID表示会话重用)

- [x] [握手协议背后的基本原理](#完整ssl会话握手协议)
	- 建立安全连接请求，包括协议版本、会话ID、密码构建、压缩方法和初始随机数
	- 服务器发送证书、密钥交换证书和证书请求，最后发送hello消息阶段结束的信号
...
- [x] [SSL的数据保护原理](#_53-ssl协议的安全性分析)
	- 数据处理过程：加密和完整性保护([加密记录的产生方法](#加密记录的产生方式))
	- 序列号的使用(不进行传输，初始化为0，随着包增加加1，并且不能对无序包进行排序)

## 防火墙和NAT

- [x] 防火墙种类、功能
	- (重点)[包过滤型](#包过滤型防火墙)和[状态检测型](#状态检测防火墙)
	- [单宿主主机](#屏蔽主机结构单宿主主机)、[双宿主主机](#双宿主主机)以及[屏蔽子网结构](#屏蔽子网结构)
	- [基本状态检测型防火墙对FTP主动和被动连接处理上的区别](#ftp在主动模式和被动模式下，经过状态监测防火墙的行为)_

- [x] [NAT基本原理](#nat的工作原理)，作用(在路由器(边界)、防火墙上实现内外地址的翻译工作)
	- [SNAT、DNAT的基本作用、内核中处理位置](https://juejin.cn/post/7145746071031906317)
	- [典型应用数据访问过程中的数据包地址和端口信息变化](#snat的工作原理)

- [x] Iptables/netfilter的基本使用
	- 原理性的，不涉及具体操作命令

- [x] 基本组网原理：交换机、路由器等的基本概念

## VPN

- [x] [VPN种类](#隧道协议和vpn类型)、功能(简单了解)

- [x] [IPSec VPN](#_73-ipsec-vpn)(重点)
...
- [x] TCP、UDP、IP协议，FTP、Email相关协议，交换机、路由器等相关设备

- [x] 基本的密码学算法
	- 对称、非对称、Hash函数、HMAC
	- 对称加密数据，公钥加密会话密钥(数字信封)、私钥签名(结合Hash)、HMAC提供数据源认证和完整性保护 
